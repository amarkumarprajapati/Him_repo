...
def _create_new_drone_csv(device_pk, ip, station_name=None):
    ...
    station = station_name or (device.station_name if device else "unknown")
    device_type = device.device_type if device else "DRONE"

    csv_dir = Path(settings.BASE_DIR) / f"{station}_{device_type}"
    csv_dir.mkdir(parents=True, exist_ok=True)
    start_ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    return csv_dir / f"DRONE_{ip}_{device_pk}_{start_ts}.csv"
...
def _get_df_csv_path(session_id, ip, session_name, station_name=None):
    ...
    if session_id not in _df_csv_paths:
        ...
        csv_dir = Path(settings.BASE_DIR) / f"{station}_DF"
        csv_dir.mkdir(parents=True, exist_ok=True)
        start_ts = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        csv_path = csv_dir / f"DF_{ip}_{session_name}_{start_ts}.csv"
